...
//              sh "echo $TF_VAR_container_version"
              sh "terraform plan -var=container_version=${GIT_COMMIT} -var=build=${GIT_BRANCH} && terraform apply -var=container_version=${GIT_COMMIT} -var=build=${GIT_BRANCH} --auto-approve"

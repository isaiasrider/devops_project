        ...
        stage('Delivery') {
         ...
         stage('Deployment') {
             steps {
              ...
              sh "export version=`echo $GIT_COMMIT|cut -c 1-5`"
              sh "terraform plan -var=container_version=${GIT_COMMIT} -var=build=${version} && terraform apply -var=container_version=${GIT_COMMIT} -var=build=${version} --auto-approve"

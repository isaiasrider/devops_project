        ...
        stage('Delivery') {
         ...
         stage('Deployment') {
             steps {
              ...
              sh "terraform plan -var=container_version=${GIT_COMMIT} -var=build=${build_name} && terraform apply -var=container_version=${GIT_COMMIT} -var=build=${build_name} --auto-approve"
